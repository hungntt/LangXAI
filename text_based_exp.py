...
def gpt_explain(original_image, explanation_image, prediction, ground_truth=None):
    # Getting the base64 string
    base64_image = convert_pil_to_base64(original_image)
    base64_image_xai = convert_pil_to_base64(explanation_image)
    prompt = (
                f"The first image is the original input image. The second image is the heatmap of {prediction} in the original image." +
                f"First, you look at the original image to identify which parts belong to {prediction} and which parts do not." +
                f"Then, you look at the second image to see the overall heatmap of {prediction}." +
                f"Can you describe the concentrated regions on the image for the classification of {prediction}?" +
                ...
                f"Your final answer should be separated by bullet points." + f"The first bullet point should give a brief overview of the heatmap." +
                ...
                f"Is the most concentrated region of predict '{prediction}' in the heatmap supportive for model to correct predict the ground truth '{ground_truth}'?") if ground_truth is not None else ""
    print(prompt)
